...
float Gearbox::torque_fwd(float torque_in) {
    return torque_in * (float)config_.gears_out / (float)config_.gears_in * config_.efficiency;
}

float Gearbox::torque_bwd(float torque_out) {
    return torque_out / config_.efficiency * (float)config_.gears_in / (float)config_.gears_out;
}
